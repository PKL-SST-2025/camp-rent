...
export default function Tracking() {
  ...
  // Saat ganti halaman (list/detail)
  createEffect(() => {
    setFade(false);
    loadData();
    setTimeout(() => setFade(true), 50); // efek fade-in
  });
  ...
  // Kalau nggak ada params.id → list
  if (!params.id) {
    const columnDefs = [
      { headerName: "Nama Barang", field: "name", flex: 1 },
      { headerName: "Tanggal Sewa", field: "date", flex: 1 },
    ...
    return (
      <div class={`max-w-6xl mx-auto p-4 transition-opacity duration-300 ${fade() ? "opacity-100" : "opacity-0"}`}>
        <h2 class="text-xl font-bold mb-4">Tracking Pesanan</h2>
        <div class="ag-theme-alpine" style={{ background: "#96AAC5", padding: "1rem", "border-radius": "1rem" }}>
          <AgGridSolid
            rowData={riwayat()}
  ...
  // Kalau ada params.id → detail
  return (
    <div class={`bg-white rounded-2xl shadow-lg p-8 max-w-3xl mx-auto mt-8 transition-opacity duration-300 ${fade() ? "opacity-100" : "opacity-0"}`}>
      {data() ? (
        <>
          <h2 class="text-2xl font-bold text-[#3F5B8B] mb-6 text-center">{data().name}</h2>
          ...
          {["Diproses", "Dikirim"].includes(data().status) && (
            <div class="text-center mb-6">
              <button onClick={updateStatus} class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition">
                Ubah status ke {data().status === "Diproses" ? "Dikirim" : "Selesai"}
              </button>
            </div>
          )}
          <div class="text-center">
            <button onClick={() => navigate("/tracking")} class="bg-[#3F5B8B] text-white px-5 py-2 rounded-md hover:bg-[#2e406b] transition">
              ← Kembali ke Tracking List
            </button>
          </div>
        </>
      ) : (
        <div class="text-center text-gray-600 text-sm mt-10">Data tidak ditemukan.</div>
      )}
    </div>
  );
}
